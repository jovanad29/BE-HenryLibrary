...
const router = Router();
const {getAll,getBook,getById,createBook,updateBook,logicalDeleteBook} = require('../controllers/catalog');
...
router.put('/delete/:id', async (req, res) => {
  ...
  try {
    if (id) {
      ...
      book
        ? res.status(200).json(book)
        : res.status(501).json({ message: 'No se encontro el libro a eliminar' });
    } else {
      let dbBooks = await getAll();
      dbBooks
        ? res.json(dbBooks)
